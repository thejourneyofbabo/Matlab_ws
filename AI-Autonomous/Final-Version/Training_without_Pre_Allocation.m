...
for episode = 1:numEpisodes
    ...
    for step_i = 1:maxStepsPerEpisode
        ...
        if bufferCounter > batchSize
            ...
            % indices = sub2ind(size(qValues), 1:batchSize, actions'); 
            if any(actions < 1 | actions > size(qValues, 1))
                error('Actions are out of range!');
            end
            indices = sub2ind(size(qValues), actions', 1:batchSize);
