...
successReward = 10;
failureReward = 0;
warmupEpisodes = 15;  % Add warm-up period
...
for episode = 1:numEpisodes
    % % Keep high exploration during warm-up
    % if episode <= warmupEpisodes
    %     epsilon = 1;  % Force full exploration during warm-up
    % end
    ...
    for step_i = 1:maxStepsPerEpisode
        ...
        if bufferCounter > batchSize %256
            ...
            if episode > warmupEpisodes
                if episode < warmupEpisodes * 1.5
                    epsilon = max(minEpsilon, epsilon * epsilonDecay);
                else
                    epsilon = max(minEpsilon2, epsilon * epsilonDecay);
                end
            else
                epsilon = max(minEpsilon, epsilon * 0.999);
            end
        end
    end
     % 학습 진행 모니터링 업데이트
     if episode > warmupEpisodes % Warmup episodes
